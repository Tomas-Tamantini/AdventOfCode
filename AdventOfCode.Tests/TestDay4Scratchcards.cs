namespace AdventOfCode.Tests
{
    ...
    {
        private readonly List<ScratchcardGame> sixCardsGame = new()
        {
            new(1,new HashSet<int>{41, 48, 83, 86, 17},new HashSet<int>{83, 86,  6, 31, 17, 9, 48, 53}),
            new(2,new HashSet<int>{13, 32, 20, 16, 61}, new HashSet<int>{61, 30, 68, 82, 17, 32, 24, 19}),
            new(3,new HashSet<int>{1, 21, 53, 59, 44}, new HashSet<int>{69, 82, 63, 72, 16, 21, 14, 1}),
            new(4,new HashSet<int>{41, 92, 73, 84, 69}, new HashSet<int>{59, 84, 76, 51, 58, 5, 54, 83}),
            new(5,new HashSet<int>{87, 83, 26, 28, 32}, new HashSet<int>{88, 30, 70, 12, 93, 22, 82, 36}),
            new(6,new HashSet<int>{31, 18, 13, 56, 72}, new HashSet<int>{74, 77, 10, 23, 35, 67, 36, 11})
        };
        ...
        {
            var scratchcards = new Scratchcards(sixCardsGame);
            Assert.Equal(13, scratchcards.TotalPoints());
        }

        [Fact]
        public void TestScratchCardWithSingleMatchingNumberGivesCopyOfSubsequentScratchCard()
        {
            var games = new List<ScratchcardGame> { BuildGame(numMatches: 4), BuildGame(numMatches: 5) };
            var scratchcards = new Scratchcards(games);
            Assert.Equal(new List<int> { 1, 2 }, scratchcards.CardsMultiplicity());
        }

        [Fact]
        public void TestScratchCardCopiesHaveCummulativeEffect()
        {
            var scratchcards = new Scratchcards(sixCardsGame);
            Assert.Equal(new List<int> { 1, 2, 4, 8, 14, 1 }, scratchcards.CardsMultiplicity());
        }
    }
}